...
def _objects(
    record: CanonicalRecord, values: list[DiscursiveElement], kind: str
...
def build_discourse_graph(record: CanonicalRecord) -> dict[str, Any]:
    ...
    for node_type, objects in groups:
        for obj in objects:
            ...
            edges.append(
                {"source": obj.object_id, "target": record.source_url, "type": "CANDIDATE_IN"}
            )
    for relation in record.analysis.relations:
        ...
        for ref in (source, target):
            if ref not in node_ids:
                ...
                node_ids.add(ref)
        edges.append({"source": source, "target": target, "type": relation.relation_type})
