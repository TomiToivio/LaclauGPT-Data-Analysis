    ...
    def write(self, rows: Iterable[Mapping[str, Any]]) -> None:
        ...
        for row in rows:
            ...
            if source_url:
                ...
                analysis_key = str(
                    payload.get("id")
                    or hash(json.dumps(payload, sort_keys=True, default=str))
                )
                identity = {
                    "project_id": self.project_id,
                    "_analysis_key": analysis_key,
                }
...
def resolved_storage_backend(settings: Settings) -> str:
    ...
    if requested == "mongodb":
        if not settings.mongo_url:
            raise ValueError("storage_backend=mongodb requires a configured MongoDB URI")
...
def artifact_store(settings: Settings):
    ...
    if settings.object_backend == "s3":
        if not settings.s3_bucket:
            raise ValueError(
                "LACLAUGPT_S3_BUCKET is required for object_backend=s3"
            )
...
def cache(settings: Settings):
    ...
    if settings.cache_backend == "redis":
        if not settings.redis_url:
            raise ValueError(
                "LACLAUGPT_REDIS_URL is required for cache_backend=redis"
            )
        return RedisCache(
            settings.redis_url,
            settings.distributed_namespace.redis_key("cache"),
        )
